...
class ImageCaptureDialog extends Component {
    ...
    _downloadImage1() {
        const recordId = this.props.rec_to_update.res_id;
        const modelName = 'medical.endoscopes';
        const fieldName = 'img1';
        const uniqueId = new Date().getTime(); // Use the current timestamp as a unique identifier

        const downloadUrl = `/web/image?model=medical.endoscopes&id=67&field=img1&unique=1716924933000`;

        const link = document.createElement('a');
        link.href = downloadUrl;
        link.setAttribute('download', 'image1.png');
