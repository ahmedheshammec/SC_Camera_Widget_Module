...
class MedicalEndoscopes(models.Model):
    ...
    dummy_file = fields.Binary(string="Dummy File", readonly=True)
    dummy_file_name = fields.Char(string="Dummy File Name", readonly=True)


    def dummyDownload(self):
        ...
        encoded_content = base64.b64encode(dummy_content)

        self.write({
            'dummy_file': encoded_content,
            'dummy_file_name': dummy_file_name,
        })

        return {
            'type': 'ir.actions.act_url',
            'url': f'/web/content/{self.id}/dummy_file/{dummy_file_name}',
            'target': 'self',
        }
        # return True
